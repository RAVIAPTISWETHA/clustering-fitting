# -*- coding: utf-8 -*-
"""
Created on Fri Jan 12 17:14:48 2024
...
def filter_data(filename, countries, indicators, start_year, end_year):
    df = pd.read_csv(filename, skiprows=4)
    cols_to_drop = ['Country Code', 'Indicator Code', 'Unnamed: 66']
    df = df.drop(cols_to_drop, axis=1)
    df = df.rename(columns={'Country Name': 'Country'})
...
def plot_future(df, countries, indicator, start_year, end_year):
    data = filter_data(df, countries, [indicator], start_year, end_year)
...
if __name__ == '__main__':
    ...
    df_years, df_countries = read_data(r"C:\Users\panne\Downloads\worlddata.csv")
    ...
    plot_future(r"C:\Users\panne\Downloads\worlddata.csv", countries, 'CO2 emissions from solid fuel consumption (% of total)', 1991, 2019)
